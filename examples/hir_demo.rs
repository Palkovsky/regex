...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let hir = RegexBuilder::new(r"\d{2,4}")
    ...
    let hir = RegexBuilder::new(r"^(?P<year>\d{4})-(?P<month>\d{2})-(?P<day>\d{2})$")
        .hir()?;
    ...
    // Bytes regex example
    let hir = regex::bytes::RegexBuilder::new(r"(?-u:.+)")
        .hir()?;
    println!("Pattern: r\"(?-u:.+)\"");
    println!("HIR: {:?}\n", hir);
    ...
    let hirs = regex::bytes::RegexSetBuilder::new([
        r"foo",
        r"bar",
        r"baz",
    ])
    ...
    let re_set = regex::bytes::RegexSetBuilder::new([""]).build_from_hirs(&hirs)?;
