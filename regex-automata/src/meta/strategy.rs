...
struct Core {
    ...
    backtrack: Arc<wrappers::BoundedBacktracker>,
    onepass: Arc<wrappers::OnePass>,
    hybrid: Arc<wrappers::Hybrid>,
    dfa: Arc<wrappers::DFA>,
}
...
impl Core {
    ...
    fn new(
        ...
        let backtrack = Arc::new(wrappers::BoundedBacktracker::new(
            &info,
            pre.clone(),
            &nfa,
        )?);
        ...
        let onepass = Arc::new(wrappers::OnePass::new(&info, &nfa));
            ...
            && !info.config().get_dfa()
        {
            (
                None,
                Arc::new(wrappers::Hybrid::none()),
                Arc::new(wrappers::DFA::none()),
            )
        } else {
            ...
            let dfa = if !info.config().get_dfa() {
                ...
                Arc::new(wrappers::DFA::new(&info, pre.clone(), &nfa, &nfarev))
            };
