...
pub(crate) mod string {
    ...
    impl RegexBuilder {
        ...
        /// Builds the HIR (High-level Intermediate Representation) for the
        pub fn hir(&self) -> Result<Hir, Error> {
            let mut syntaxc = self.builder.syntaxc.clone();
            syntaxc.utf8(true);
            let mut hirs = meta::Builder::new()
                .syntax(&syntaxc)
                .build_many_hirs(&[self.builder.pats[0].as_str()])
                .map_err(Error::from_meta_build_error)?;
    ...
    impl RegexSetBuilder {
        ...
        /// Builds the HIR (High-level Intermediate Representation).
...
pub(crate) mod bytes {
    ...
    impl RegexBuilder {
        ...
        /// Returns a Hir (High-level Intermediate Representation) for the pattern.
        pub fn hir(&self) -> Result<Hir, Error> {
            let mut syntaxc = self.builder.syntaxc.clone();
            syntaxc.utf8(false);
            let mut hirs = meta::Builder::new()
        ...
        /// Builds a `Regex` from a HIR (High-level Intermediate Representation).
        pub fn build_from_hir(&self, hir: &Hir) -> Result<Regex, Error> {
            let mut metac = self.builder.metac.clone();
            metac.match_kind(MatchKind::LeftmostFirst).utf8_empty(false);
    ...
    impl RegexSetBuilder {
        ...
        /// Builds the HIR (High-level Intermediate Representation) for patterns.
        pub fn hirs(&self) -> Result<Vec<Hir>, Error> {
            let mut syntaxc = self.builder.syntaxc.clone();
            syntaxc.utf8(false);
            let pats: Vec<&str> = self.builder.pats.iter().map(|s| s.as_str()).collect();
